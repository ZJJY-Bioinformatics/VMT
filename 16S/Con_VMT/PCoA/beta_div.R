...
ggplot(PC_plot, aes(PC1, PC2)) + 
  ...
  theme_minimal() + 
  stat_ellipse(aes(PC1, PC2, fill = Day), geom = "polygon", show.legend = F, alpha = 0.1) + 
  scale_color_distiller(palette = "Spectral", direction = 1, guide = guide_legend(), label = c("D3", "D7", "D30", "D42")) + 
  scale_fill_brewer(palette = "Spectral", direction = 1) +
  #scale_fill_manual(values = c(pal[c(2,4,6,8,10,12)])) + 
  #scale_color_manual(values = c(pal[c(2,4,6,8,10,12)])) + 
  scale_shape_manual(values = c(15, 16)) + 
  labs(color = "Day",
       x = paste0("PC1 (", proportion[1], "%)"), 
       y = paste0("PC2 (", proportion[2], "%)")) +
  theme(aspect.ratio = 1)
#ggsave("bray_curtis_PCoA.pdf", width = 4, height = 4)


ggplot(PC_plot, aes(PC1, PC2)) + 
  geom_point(aes(fill = Day, shape = Group), show.legend = T) +
  theme_minimal() + 
  #stat_ellipse(aes(PC1, PC2, fill = Day), geom = "polygon", show.legend = F, alpha = 0.1) + 
  #scale_color_distiller(palette = "Spectral", direction = 1, guide = guide_legend(), label = c("D3", "D7", "D30", "D42")) + 
  #scale_fill_brewer(palette = "Spectral", direction = 1) +
  ...
  labs(fill = "Day",
       x = paste0("PC1 (", proportion[1], "%)"), 
       y = paste0("PC2 (", proportion[2], "%)")) +
  theme(aspect.ratio = 1, panel.border = element_rect(color = "black", fill = "transparent")) +
  guides(fill = guide_legend(override.aes = list(alpha = 1)))



ggplot(PC_plot, aes(PC1, PC2)) + 
  geom_point(aes(color = Day, shape = Group), show.legend = T, size = 2) +
  theme_minimal() + 
  #stat_ellipse(aes(PC1, PC2, fill = Day), geom = "polygon", show.legend = F, alpha = 0.1) + 
  #scale_color_distiller(palette = "Spectral", direction = 1, guide = guide_legend(), label = c("D3", "D7", "D30", "D42")) + 
  #scale_fill_brewer(palette = "Spectral", direction = 1) +
  ...
  scale_shape_manual(values = c(15, 16)) + 
  #scale_fill_manual(values = c("#eff3ff", "#bdd7e7", "#6baed6", "#2171b5")) + 
  scale_color_manual(values = c("#BBE9E9", "#89B0E7", "#5C7EBE", "#3B4E76")) + 
  labs(color = "Day",
       x = paste0("PC1 (", proportion[1], "%)"), 
       y = paste0("PC2 (", proportion[2], "%)")
  ) +
...
ggsave("fig1a.pdf", width = 4, height = 3)
...
group = PC_plot %>% column_to_rownames(var = "#SampleID")
adonis2(bc ~ Day, group, permutations = 999)


pal = RColorBrewer::brewer.pal(12, "Paired")
PC_plot %>% mutate(color = factor(str_c(Group, Time_point))) %>% 

  

ggscatter(x = "PC1", y = "PC2", color = "color", fill = "color", shape = "Group", mean.point = T, ellipse = T, mean.point.size = 5,  ellipse.border.remove = T) + 
  stat_stars(aes(color = color), alpha = 0.2) + 
  scale_color_manual(values = c(pal[c(1,3,5,7,9,11)], pal[c(2,4,6,8,10,12)])) + 
  scale_fill_manual(values = c(pal[c(1,3,5,7,9,11)], pal[c(2,4,6,8,10,12)]))+
  scale_shape_manual(values = c(15,16)) +
  theme_minimal() + 
...
coor_summary = PC_plot %>% group_by(Group, Day) %>% 
  summarise(PC1.se = sd(PC1)/sqrt(n()), 
            ...
            PC2.sd = sd(PC2)) 
ggplot() + 
  #geom_point(data = PC_plot, aes(PC1, PC2, color = Day, shape = Group)) +
...
ggsave("bray_curtis_PCoA_errorbar.pdf", width = 4, height = 4)


data = read.pc("bdiv/bray_curtis_pc.txt") 
coor = data$pc_data %>% 
  select(1:3) %>% 
  as.data.frame() %>% 
  rownames_to_column(var = "#SampleID") %>% 
...
ggsave("bray_curtis_所有样本PCoA.pdf", width = 5, height = 5)
...
#pdf("3D_PCoA.pdf", width = 5.2, height = 5.2)
p = scatterplot3d(PC_plot_3d$PC1, PC_plot_3d$PC2, PC_plot_3d$PC3, 
              ...
              grid = F, box = F, pch = "", 
              col.axis = "black")
addgrids3d(PC_plot_3d$PC1, PC_plot_3d$PC2, PC_plot_3d$PC3, grid = c("xy", "xz", "yz"))
p$points3d(PC_plot_3d$PC1, PC_plot_3d$PC2, PC_plot_3d$PC3, pch = PC_plot_3d$shape, bg = colors)
legend("topright", legend = levels(PC_plot_3d$Group), 
       pch = c(21, 22))
legend("right", legend = levels(PC_plot_3d$Day), col = colormap, pch = 15)
#dev.off()
#
#pdf("3D_PCoA2.pdf", width = 6, height = 6)
p = scatterplot3d(PC_plot_3d$PC1, PC_plot_3d$PC2, PC_plot_3d$PC3, 
...
library(rgl)
plot3d(PC_plot_3d$PC1, PC_plot_3d$PC2, PC_plot_3d$PC3, 
       col = colors)
...
D3 = pcoa("../../beta/D3/bray_curtis_pc.txt", 0.02, 0.14)
D7 = pcoa("../../beta/D7/bray_curtis_pc.txt", 0.02, 0.18)
D30 = pcoa("../../beta/D30/bray_curtis_pc.txt", 0.03, 0.07)
D42 = pcoa("../../beta/D42/bray_curtis_pc.txt", 0.04, 0.009)

P = D3 +D7 + D30 + D42 + plot_layout(guides = 'collect') &
  theme(legend.position = "bottom")
P

ggsave("supp_fig1_各时间点PCoA_bray_curtis_minimal.pdf", width = 5, height = 5)
...
p1 = ggplot(PC_plot, aes(PC1, y = Day)) + 
  ...
  geom_text(data = pvalue1, mapping = aes(y = Day, label = paste0("P=", p)), x = 0.35, hjust = .5, vjust = -1.1) + 
  #scale_fill_manual(values = c('#1f77b4', '#ff7f0e')) + 
  scale_fill_manual(values = c("#7BC2CE", "#8562CC")) + 
  xlab("Bray-Curtis Dissimilarity (PCoA1)") + 
  theme(aspect.ratio = 1)
ggsave("fig1c.pdf", width = 5, height = 4)
p2 = ggplot(PC_plot, aes(PC2, y = Day)) + 
  geom_density_ridges2(aes(fill=Group), alpha = 0.6) +
  theme_ridges() + 
  theme(axis.title.x = element_text(hjust = 0.5), 
        axis.title.y = element_text(hjust = 0.5)) + 
  geom_text(data = pvalue2, mapping = aes(y = Day, label = paste0("P=", p)), x = 0.3, hjust = .5, vjust = -1.1) + 
  #scale_fill_manual(values = c('#1f77b4', '#ff7f0e')) + 
  scale_fill_manual(values = c("#7BC2CE", "#8562CC")) + 
  xlab("Bray-Curtis Dissimilarity (PCoA2)")
ggarrange(p1, p2, common.legend = T)
ggsave("Bray-Curtis Dissimilarity_PC1-2峰峦图-color2.pdf", width = 8,height = 4)
...
func1 = function(day){
  pc = read.pc(paste0("D", day, "/bray_curtis_pc.txt"))
  PC_coor = pc$pc_data %>% 
    select(1:3) %>% 
    as.data.frame() %>% 
    rownames_to_column(var = "#SampleID")
...
each_day_PC = map_dfr(c(1,2,3,7,30,42), ~func1(.x)) %>% 
  inner_join(meta, by = "#SampleID") %>% 
  mutate(Day = fct_relevel(Day, c("D3", "D7", "D30", "D42")))

pvalue1 = each_day_PC %>% group_by(Day) %>% 
  rstatix::wilcox_test(PC1 ~ Group)
pvalue2 = each_day_PC %>% group_by(Day) %>% 
  rstatix::wilcox_test(PC2 ~ Group)

p1 = ggplot(each_day_PC, aes(PC1, y = Day)) + 
  geom_density_ridges2(aes(fill=Group), alpha = 0.6) +
  theme_ridges() + 
  theme(axis.title.x = element_text(hjust = 0.5), 
        axis.title.y = element_text(hjust = 0.5)) + 
  geom_text(data = pvalue1, mapping = aes(y = Day, label = paste0("P=", p)), x = 0.65, hjust = .5, vjust = -1.1) + 
  #scale_fill_manual(values = c('#1f77b4', '#ff7f0e')) + 
  scale_fill_manual(values = c("#7BC2CE", "#8562CC")) + 
  xlab("braycurtis (PCoA1)")
p1
p2 = ggplot(each_day_PC, aes(PC2, y = Day)) + 
  geom_density_ridges2(aes(fill=Group), alpha = 0.6) +
  theme_ridges() + 
  theme(axis.title.x = element_text(hjust = 0.5), 
        axis.title.y = element_text(hjust = 0.5)) + 
  geom_text(data = pvalue2, mapping = aes(y = Day, label = paste0("P=", p)), x = 0.65, hjust = .5, vjust = -1.1) + 
  #scale_fill_manual(values = c('#1f77b4', '#ff7f0e')) + 
  scale_fill_manual(values = c("#7BC2CE", "#8562CC")) + 
  xlab("braycurtis (PCoA2)")
p2
ggarrange(p1, p2, common.legend = T)
cowplot::plot_grid(p1, p2, align = "vh")
p1 + p2 + 
  plot_layout(guides = "collect")
p1 %>% aplot::insert_right(p2)
p1
ggsave("PC1峰峦图-color2_每一天的PCOA.pdf", width = 5,height = 4)
