...
fdr0.25 = uni %>% filter(qval < 0.25) %>% 
  pull(feature)

uni_res2 = uni %>% filter(feature %in% fdr0.25) %>% 
...
fdr0.25 = multi %>% filter(p_adjust < 0.25) %>% 
  pull(feature)

multi_res2 = multi %>% filter(feature %in% fdr0.25) %>% 
...
label = multi %>% filter(p_adjust<0.25) %>% 
  mutate(labels = if_else(coef > 0, "*", "*"))
