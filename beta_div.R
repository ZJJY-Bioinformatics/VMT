...
ggplot(PC_plot, aes(PC1, PC2)) + 
  ...
  scale_color_distiller(palette = "Spectral", direction = 1, guide = guide_legend(), label = c("D1", "D2", "D3", "D7", "D30", "D42")) + 
...
ggplot() + 
  ...
  scale_color_brewer(palette = "Spectral", direction = 1, guide = guide_legend(), label = c("D1", "D2", "D3", "D7", "D30", "D42")) +
...
coor = coor %>% inner_join(centroid, by = c("Group", "Day")) %>% 
  mutate(color = factor(str_c(Group, Time_point)))
...
ggplot() + 
  ...
  scale_color_manual(values = c(pal[c(1,3,5,7,9,11)], pal[c(2,4,6,8,10,12)])) + 
  scale_fill_manual(values = c(pal[c(1,3,5,7,9,11)], pal[c(2,4,6,8,10,12)]),
                    label = c(str_c(c("Con"), c("D1", "D2", "D3", "D7", "D30", "D42"), sep = "_"), str_c(c("VMT"), c("D1", "D2", "D3", "D7", "D30", "D42"), sep = "_")))+
  theme_minimal() +
  labs(
    fill = "",
    x = paste0("PC1 (", proportion[1], "%)"),
...
D1 = pcoa("D1/bray_curtis_pc.txt", 0.02, 0.19)
D2 = pcoa("D2/bray_curtis_pc.txt", 0.04, 0.003)
D3 = pcoa("D3/bray_curtis_pc.txt", 0.02, 0.14)
D7 = pcoa("D7/bray_curtis_pc.txt", 0.02, 0.18)
D30 = pcoa("D30/bray_curtis_pc.txt", 0.03, 0.07)
D42 = pcoa("D42/bray_curtis_pc.txt", 0.04, 0.009)

P = D1 + D2 + D3 +D7 + D30 + D42 + plot_layout(guides = 'collect')
P

ggsave("各时间点PCoA_bray_curtis_minimal.pdf", width = 9, height = 6)
...
p1 = ggplot(PC_plot, aes(PC1, y = Day)) + 
  ...
  geom_text(data = pvalue1, mapping = aes(y = Day, label = paste0("P=", p)), x = 0.55, hjust = .5, vjust = -1.1) + 
...
p2
ggsave("PC2峰峦图-color2_每一天的PCOA.pdf", width = 5,height = 4)
