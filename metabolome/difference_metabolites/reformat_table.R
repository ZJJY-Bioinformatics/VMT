library(tidyverse)
mtblt_res = read_tsv("各代谢物在各时间点wilcox检验.tsv")
...
metabolites = read_tsv("../../../metabolome/metabolome-tsfm.txt") %>% 
  filter(Sample %in% map$`#SampleID`)
pvalue = function(day){
...
d3_kruskal = read_tsv("genus_kruskal_wallis/Day3_kruskal_wallis.txt") %>% 
...
d7_kruskal = read_tsv("genus_kruskal_wallis/Day7_kruskal_wallis.txt")  %>% 
  filter(!str_detect(OTU, ".*Other$")) %>% 
  filter(!str_detect(OTU, ".*g__$")) %>% 
  select(OTU, P, VMT_mean, VD_mean, Con_mean) %>% 
  rename(kruskal_P = P) 
...
d30_kruskal = read_tsv("genus_kruskal_wallis/Day30_kruskal_wallis.txt") %>% 
  filter(!str_detect(OTU, ".*Other$")) %>% 
  filter(!str_detect(OTU, ".*g__$")) %>% 
  select(OTU, P, VMT_mean, VD_mean, Con_mean) %>% 
  rename(kruskal_P = P) 
d30_kruskal$OTU = taxa_shortname(d30_kruskal$OTU)
d42_kruskal = read_tsv("genus_kruskal_wallis/Day42_kruskal_wallis.txt")  %>% 
  filter(!str_detect(OTU, ".*Other$")) %>% 
  filter(!str_detect(OTU, ".*g__$")) %>% 
  select(OTU, P, VMT_mean, VD_mean, Con_mean) %>% 
  rename(kruskal_P = P) 
